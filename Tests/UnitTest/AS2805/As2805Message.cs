...
namespace UnitTest.AS2805
{
    /// <summary>
    /// http://www.splinter.com.au/as2805-like-iso8583-financial-message-parser/
    /// </summary>
    public class As2805Message
    {
        ...
        public string de35_Track2;        // DE35: Track 2 (z37)
        ...
        public As2805Message(byte[] buf)
        {
            ...
            mti = String.Concat(buf.Take(2).Select(c=>c.ToString("X2")));
            ...
            if ((primaryBitmap[0] & 128) > 0) de1_SecondaryBitmap = ReadFixRaw(buf, ref offset, 8);//2
            if ((primaryBitmap[0] & 64) > 0) de2_PAN = ReadVar2Packed(buf, ref offset, 19);//
            if ((primaryBitmap[0] & 32) > 0) de3_ProcCode = ReadFixPacked(buf, ref offset, 6); // 13
            if ((primaryBitmap[0] & 16) > 0) de4_AmtTxn = ReadFixPacked(buf, ref offset, 12);// 19
            ...
            if ((primaryBitmap[0] & 2) > 0) de7_TransDttm = ReadFixPacked(buf, ref offset, 10);//24
            ...
            if ((primaryBitmap[1] & 32) > 0) de11_STAN = ReadFixPacked(buf, ref offset, 6);//27
            if ((primaryBitmap[1] & 16) > 0) de12_TimeLocal = ReadFixPacked(buf, ref offset, 6);//30
            if ((primaryBitmap[1] & 8) > 0) de13_DateLocal = ReadFixPacked(buf, ref offset, 4);//32
            if ((primaryBitmap[1] & 4) > 0) de14_DateExpiry = ReadFixPacked(buf, ref offset, 4);
            if ((primaryBitmap[1] & 2) > 0) de15_DateSetl = ReadFixPacked(buf, ref offset, 4);//34
            ...
            if ((primaryBitmap[2] & 64) > 0) de18_MerchType = ReadFixPacked(buf, ref offset, 4);//36
            ...
            if ((primaryBitmap[3] & 128) > 0) de25_PosCondCode = ReadFixPacked(buf, ref offset, 2);//38
            ...
            //if ((primaryBitmap[3] & 16) > 0) de28_AmtTxnFee = ReadFixPacked(buf, ref offset, 8);//39
            if ((primaryBitmap[3] & 16) > 0) de28_AmtTxnFee = ReadAnFixPacked(buf, ref offset, 8 );//39
            ...
            if ((primaryBitmap[3] & 1) > 0) de32_AcqInstIdCode = New_ReadVar2Packed(buf, ref offset, 11);//
            //if ((primaryBitmap[3] & 1) > 0) de32_AcqInstIdCode = ReadVar2Packed(buf, ref offset, 11);//
            if ((primaryBitmap[4] & 128) > 0) de33_FwdInstIdCode = New_ReadVar2Packed(buf, ref offset, 11);
            ...
            if ((primaryBitmap[4] & 32) > 0) de35_Track2 = p35_ReadVar2Packed(buf, ref offset, 37);
            ...
            if ((de1_SecondaryBitmap[4] & 16) > 0) de100_RecvInstIdCode = New_ReadVar2Packed(buf, ref offset, 11);
        ...
        string New_ReadVar2Packed(byte[] buf, ref int offset, int unused)
        {
            int len = int.Parse(buf[offset].ToString("X2"));
            int bytes = len;

            if (bytes % 2 == 0)
                bytes /= 2;
            else
                bytes = (bytes + 1) / 2;// The +1 is so it rounds up
            byte b;
            StringBuilder sb = new StringBuilder(len);
        ...
        private string p35_ReadVar2Packed(byte[] buf, ref int offset, int p)
        {
            int len = int.Parse(buf[offset].ToString("X2"));
            int bytes = len;
        ...
        private string ReadAnFixPacked(byte[] buf, ref int offset, int len)
        {
            int bytes = (len + 1)/2; // The +1 is so it rounds up
            byte b;
            StringBuilder sb = new StringBuilder(len);
