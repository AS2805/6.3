...
namespace SwitchLink.CoreNode.Configuration
{
    class Config
    {
        ...
        public static string HostIpAddress
        {
            get
            {
                string addrString = ConfigurationManager.AppSettings["app:HostIpAddress"];
                if (string.IsNullOrEmpty(addrString))
                {
                    IPHostEntry ipHostInfo = Dns.Resolve(Dns.GetHostName());
                    addrString = ipHostInfo.AddressList[0].ToString();
                ...
                {
                    //Invalid IP
                    throw new InvalidOperationException("Invalid Host IP address");
                }
            }
        }
        ...
        public static int HostPort
        {
            get
            {
                string port = ConfigurationManager.AppSettings["app:HostPort"];
                if (string.IsNullOrEmpty(port))
                    port = "0";
                return int.Parse(port);
            }
